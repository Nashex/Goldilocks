...
public class CommandCase extends Command {
    public CommandCase() {
        setAliases(new String[] {"case"});
        setEligibleRoles(new String[] {"security","rl"});
        setGuildRank(3);
    ...
    public void execute(Message msg, String alias, String[] args) {
        ...
        if (args.length == 0) {
            Utils.errorMessage("Failed to Retrieve Case File", "No case id provided", textChannel, 10L);
            return;
        }

        if (!PunishmentConnector.caseExists(args[0], msg.getGuild().getId())) {
            Utils.errorMessage("Failed to Retrieve Case File", "A case does not exist for the following case id: " + args[0], textChannel, 10L);
            return;
        }
        msg.delete().queue();

        new CaseFileCpInstance(args[0], textChannel, msg.getMember());

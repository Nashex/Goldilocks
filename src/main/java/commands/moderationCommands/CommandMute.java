...
import moderation.punishments.controlpanels.MuteCP;
...
public class CommandMute extends Command {
    ...
    public void execute(Message msg, String alias, String[] args) {
        ...
        if (args.length < 2) {
            msg.delete().queue();
            Utils.errorMessage("Failed to Issue Mute", "Not enough arguments to mute!", msg.getTextChannel(), 10L);
            return;
        }
        ...
        if (members.isEmpty()) {
            Utils.errorMessage("Failed to Issue Mute", "Unable to find user", msg.getTextChannel(), 10L);
            return;
        };

        new MuteCP(msg, args, members.toArray(new Member[0]));
    ...
    public EmbedBuilder getInfo() {
        ...
        embedBuilder.setTitle("Command: Mute");
        ...
        commandDescription += "\nMutes a given user. The time field is optional as it can be" +
